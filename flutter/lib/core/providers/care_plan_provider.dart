...
/// 按分类分组的指标
final metricsByCategoryProvider = Provider<Map<CareCategory, List<CareMetric>>>((ref) {
...
final wellnessScoreProvider = FutureProvider<WellnessScore>((ref) async {
  ...
  double calculateCategoryScore(CareCategory category) {
    final categoryMetrics = metrics.where((m) => m.category == category && m.isEnabled).toList();
  ...
  final overall = ((wellnessScore + nutritionScore + enrichmentScore + careScore) / 4 * healthPenalty).clamp(0.0, 100.0);

  // 生成改进建议
  final improvements = <String>[];
  if (wellnessScore < 70) improvements.add('Track health metrics more regularly');
  if (nutritionScore < 70) improvements.add('Log meals and water intake daily');
  if (enrichmentScore < 70) improvements.add('Increase activity and enrichment time');
  if (careScore < 70) improvements.add('Keep up with grooming and care routines');
  if (pet.isSick) improvements.add('Focus on recovery - follow vet recommendations');
...
class CarePlanNotifier extends StateNotifier<AsyncValue<void>> {
  ...
  /// 更新指标
  Future<void> updateMetric(String metricId, Map<String, dynamic> updates) async {
    if (AppConfig.useLocalMode) {
      final localStorage = _ref.read(localStorageProvider);
  ...
  Future<void> toggleMetric(String metricId, bool isEnabled) async {
    await updateMetric(metricId, {'is_enabled': isEnabled, 'updated_at': DateTime.now().toIso8601String()});
  }
...
final carePlanNotifierProvider = StateNotifierProvider<CarePlanNotifier, AsyncValue<void>>((ref) {
...
bool _shouldShowToday(CareMetric metric, DateTime today) {
  switch (metric.frequency) {
    ...
    case MetricFrequency.twiceWeekly:
      ...
      return today.weekday == DateTime.monday || today.weekday == DateTime.thursday;
